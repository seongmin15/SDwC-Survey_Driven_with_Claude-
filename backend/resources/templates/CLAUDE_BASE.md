...
- Each task is executed by a sub-agent scoped to the target service's skill file.
- Sub-agents may only modify files within their service directory.
- Main agent reviews sub-agent results before proposing to the user.
- Cross-service tasks (Service: common) are handled directly by the main agent.

### Parallel Execution

- Tasks with no inter-service dependencies may run in parallel.
- The number of tasks in Review state must not exceed the WIP Limit.
- Main agent determines dependency phases before dispatching parallel tasks.

---

{{/if}}
{{#if collaboration.model_routing == opus_sonnet}}
## Model Routing
...
- Work is organized in time-boxed Sprints (length defined in `docs/00-collaboration-model.md`).
- Sprint Planning: AI proposes a Sprint Goal and selects tasks from Backlog. User approves.
- Mid-sprint scope changes require user approval.
- Sprint Review: AI summarizes completed/incomplete tasks at sprint end.
- Incomplete tasks are not auto-carried; user decides whether to move them to the next Sprint.
...
{{#if git.pr_by == ai}}
4. Run `gh pr create --base develop --title "T<NNN>: <title>"` to create a PR.
{{else}}
...
3. `git push origin task/*`.
{{#if git.pr_by == ai}}
4. Run `gh pr create --base main --title "T<NNN>: <title>"` to create a PR.
{{else}}
4. Notify user that the branch is ready for PR.
{{/if}}
5. **Stop and wait** for user to merge the PR into main.
6. After user confirms merge: `git checkout main` → `git pull origin main`.
7. Delete finished branch: `git branch -d task/*`.
{{/if}}
...
1. Update `docs/09-task-backlog.md`: set Status to Done, write Result.
2. Commit all changes (code + backlog update) on `feature/*` branch.
...
5. **Stop and wait** for user to merge the PR into develop.
6. After user confirms merge: `git checkout develop` → `git pull origin develop`.
7. Delete finished branch: `git branch -d feature/*`.
{{/if}}
...
2. Fill detail documents (each **phase** requires approval — present all docs in a phase together):
   ...
   - **Per-service phases** — scan `docs/` for service documents (20s, 30s, 40s, 50s, 60s) and process each range in number order:
     - **Design phase**: X0, X1 documents (present together)
     - **Verification phase**: X2–X4 documents (requires the Design phase of the same range complete)
   - **Deployment phase**: 10-* template — if present (after all service phases)
   > Non-MSA: `docs/10~69-*.md`
   > MSA: `docs/services/{service-name}/20~69-*.md`
   > **Rules**:
...
{{#if collaboration.methodology == scrum}}
   - Propose Sprint 1 Goal and assign tasks from Backlog to Sprint 1.
{{/if}}
...
{{/if}}
{{#if git.branch_strategy == main_feature}}
7. Run `git init` + generate `.gitignore` + initial commit on main.
   - `.gitignore` must include `.sdwc/`, language-specific ignores (e.g., `__pycache__/`, `node_modules/`, `*.pyc`), `.env`, and IDE files.
   - All subsequent work happens on `main` or `task/*` branches from main.
{{/if}}
{{#if git.branch_strategy == trunk}}
7. Run `git init` + generate `.gitignore` + initial commit on main.
   - `.gitignore` must include `.sdwc/`, language-specific ignores (e.g., `__pycache__/`, `node_modules/`, `*.pyc`), `.env`, and IDE files.
...
7. Run `git init` + generate `.gitignore` + initial commit on master.
   - `.gitignore` must include `.sdwc/`, language-specific ignores (e.g., `__pycache__/`, `node_modules/`, `*.pyc`), `.env`, and IDE files.
   - Immediately create and switch to `develop` branch: `git checkout -b develop`.
   - All subsequent work happens on `develop` or `feature/*` branches from develop.
{{/if}}
...
{{#if collaboration.methodology == scrum}}
4. Check current Sprint status. If Sprint period has ended, run Sprint Review first.
5. Read relevant docs for the next pending task.
{{#if git.branch_strategy == master_develop_task}}
6. Verify current git branch: `git branch` → confirm on `develop`.
{{/if}}
{{#if git.branch_strategy == main_feature}}
6. Verify current git branch: `git branch` → confirm on `main`.
{{/if}}
{{#if git.branch_strategy == trunk}}
6. Verify current git branch: `git branch` → confirm on `main`.
{{/if}}
...
6. Verify current git branch: `git branch` → confirm on `develop`.
{{/if}}
7. Propose the next action based on Sprint and backlog state.
{{else}}
...
{{/if}}
{{#if git.branch_strategy == main_feature}}
5. Verify current git branch: `git branch` → confirm on `main`.
{{/if}}
{{#if git.branch_strategy == trunk}}
5. Verify current git branch: `git branch` → confirm on `main`.
{{/if}}
